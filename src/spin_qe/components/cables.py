...
Tc_super=10 # critical temperature of the SC wire
...
def integratedlambda_304SS(T1,T2):
    ...
    if(T1<T2):
        ...
        sign=-1
    if T1 <= 4 and T2 <= 4:
        res=-0.0340495*T1**2 + 0.0340495*T2**2
        return res*sign
    if T1 <= 4 and T2 > 4:
...
def integratedlambda_Kapton(T1,T2):
    ...
    if T1==T2:
        return 0
    if T1 <= 4 and T2 <= 4:
        res=-0.002948717948717949*T1**1.56 + 0.002948717948717949*T2**1.56
        return res*sign
    if T1 <= 4 and T2 > 4:
    ...
    if T1 > 4 and T2 > 4:
        ...
        return res*sign
# Heat flow per wires:   
def heatFlow(T1,T2,r1,integrated_lambda1,r2,integrated_lambda2,r3,integrated_lambda3):
...
def conduction_T1_To_T2_fixedwire(T1,T2,wire):
    ...
    return heatFlow(T1,T2,r1,integrated_lambda1,r2,integrated_lambda2,r3,integrated_lambda3)
...
def sum_conduction_power(cryo: Cryo) -> float:
    total_power = 0.0
...
def main():
    ...
    cryo_instance = Cryo(Tq=0.04, temps=[0.1, 4, 300], attens=[2, 10, 30], Si_abs=0.0, cables_atten=30)
